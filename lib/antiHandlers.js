...
export async function handleAntiSystems(m, conn, isAdmin, isOwner, isRAdmin, isBotAdmin, isPrems, commandExecuted) {
  ...
  const isOwnBot = (sender) => {
    ...
    if (cleanSender === mainBotJid) {
      console.log(`🤖 Anti-skip: Mensaje del bot principal local (${cleanSender})`)
      return true
    }

   
    if (global.mainBotJid && cleanSender === global.mainBotJid) {
      console.log(`🤖 Anti-skip: Mensaje del bot principal global (${cleanSender}) - MainBotJID: ${global.mainBotJid}`)
      return true
    }

   
    if (global.conns && Array.isArray(global.conns)) {
      const isSubBot = global.conns.some(bot => {
        const botJid = bot.user?.jid?.split('@')[0] || bot.user?.id?.split('@')[0]
        return cleanSender === botJid
      })

      if (isSubBot) {
        console.log(`🤖 Anti-skip: Mensaje de sub-bot (${cleanSender})`)
        return true
      }
    }
