...
function getBotInfo() {
  try {
    ...
    if (fs.existsSync(pluginsDir)) {
      ...
      botInfo.push(`=== PLUGINS DISPONIBLES ===\n${plugins.join(', ')}\n`)
    }
    
    // Leer package.json para info básica
    const packagePath = path.join(process.cwd(), 'package.json')
    if (fs.existsSync(packagePath)) {
      const pkg = JSON.parse(fs.readFileSync(packagePath, 'utf8'))
      botInfo.push(`=== INFORMACIÓN DEL BOT ===\nNombre: ${pkg.name}\nVersión: ${pkg.version}\nDescripción: ${pkg.description}\n`)
    }
    
    return botInfo.join('\n')
  } catch (error) {
    console.error('Error obteniendo info del bot:', error)
    return 'Información del bot no disponible temporalmente.'
  }
}
