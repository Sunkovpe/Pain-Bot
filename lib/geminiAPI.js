...
const GEMINI_API_KEYS = [
  'AIzaSyCo4U2rt_dtma5KJa_7a-0StZk3B-jVr0g',
  'AIzaSyDh8vCrHwR54YhaQaM6UWuRPMYEbOTCn3o',
  'AIzaSyDjzLxCle1eVpIiFy8x7oN88cdHIGuA7ns'
]
...
/**
 * Obtiene información completa del bot leyendo archivos
...
function getBotInfo() {
  try {
    ...
    // Leer main-menu.js y extraer comandos sin código
    const mainMenuPath = path.join(process.cwd(), 'plugins', 'main-menu.js')
    if (fs.existsSync(mainMenuPath)) {
      const mainMenuContent = fs.readFileSync(mainMenuPath, 'utf8')
      // Extraer líneas que parecen descripciones de comandos
      const commandLines = mainMenuContent.split('\n')
        .filter(line => line.includes('»') || line.includes('•') || line.includes('comando'))
        .slice(0, 20) // Limitar
      botInfo.push(`=== FUNCIONALIDADES DEL BOT ===\n${commandLines.join('\n')}\n`)
    }
    
    // Lista de plugins sin .js
    const pluginsDir = path.join(process.cwd(), 'plugins')
    if (fs.existsSync(pluginsDir)) {
      const plugins = fs.readdirSync(pluginsDir)
        .filter(file => file.endsWith('.js') && !file.includes('menu'))
        .map(file => file.replace('.js', '').replace(/-/g, ' '))
      botInfo.push(`=== COMANDOS DISPONIBLES ===\n${plugins.join(', ')}\n`)
    }
    
    // Info básica sin detalles técnicos
    const packagePath = path.join(process.cwd(), 'package.json')
    if (fs.existsSync(packagePath)) {
      const pkg = JSON.parse(fs.readFileSync(packagePath, 'utf8'))
      botInfo.push(`=== SOBRE EL BOT ===\nSoy ${pkg.name}, un bot de WhatsApp creado por Sunkovv. Versión ${pkg.version}.\n`)
    }
    
    return botInfo.join('\n')
  } catch (error) {
    console.error('Error obteniendo info del bot:', error)
    return 'Información del bot disponible próximamente.'
  }
}
...
- Evita frases como "Hola de nuevo" o "Hola otra vez"
- Si preguntan sobre comandos, menú o funcionalidades del bot, usa la información proporcionada abajo
- Explica los comandos y funcionalidades de forma natural, sin mostrar código o sintaxis de comandos
- No reveles el código fuente o archivos del bot
- Responde como si estuvieras explicando verbalmente
- Si el usuario pide ejecutar un comando específico (como reproducir música, banear, etc.), incluye [COMANDO: nombre_del_comando argumentos] al final de tu respuesta para ejecutarlo automáticamente
