  ...
  try {
    ...
    if (response && response.length > 0) {
      ...
      if (commandMatch) {
        ...
        const fakeMessage = Object.create(null)
        Object.assign(fakeMessage, {
          chat: m.chat,
          sender: m.sender,
          fromMe: m.fromMe,
          isGroup: m.isGroup,
          ...
          reply: m.reply || (() => {}),
          isFake: true, // Marcar como mensaje simulado
        })
        Object.setPrototypeOf(fakeMessage, Object.prototype)
        
        // Llamar al handler con el mensaje simulado
        try {
          const handlerModule = await import('../../handler.js')
          await handlerModule.handler.call(conn, fakeMessage)
        } catch (error) {
