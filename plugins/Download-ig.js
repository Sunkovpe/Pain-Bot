...
let handler = async (m, { conn, text, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (!json.status || !json.result?.data?.length) {
      return conn.sendMessage(m.chat, {
        text: `⚠︎ No se pudo obtener el contenido.`,
        contextInfo: { ...rcanal?.contextInfo }
      }, { quoted: m })
    }
    ...
    let captionMsg = `╭───「 ✦ 𝗜𝗡𝗦𝗧𝗔𝗚𝗥𝗔𝗠 ✦ 」\n│\n`
    captionMsg += `│  *Usuario:* ${profile?.username || '-'}\n`
    captionMsg += `│  *Nombre:* ${profile?.full_name || '-'}\n`
    captionMsg += `│  *Likes:* ${statistics?.like_count || 0}\n`
    captionMsg += `│  *Comentarios:* ${statistics?.comment_count || 0}\n`
    captionMsg += caption?.text ? `│\n│  *Descripción:* ${caption.text}\n` : ''
    captionMsg += `│\n╰───「 ✦ ${global.packname} ✦ 」`
