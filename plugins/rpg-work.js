...
let handler = async (m, { conn, usedPrefix, command }) => {
  try {
    ...
    if (trabajoSeleccionado.tipo === 'positivo') {
      ...
      user.coins += cantidad
      resultado = `✅ ¡Trabajo completado con éxito!`
      emoji = '💼'
    } else {
    ...
    let txt = `╭─「 ✦ 💼 ᴛʀᴀʙᴀᴊᴏ ✦ 」─╮\n`
    txt += `│\n`
    txt += `╰➺ ✧ ${emoji} *Trabajo:* ${trabajoSeleccionado.descripcion}\n`
    txt += `╰➺ ✧ ${resultado}\n`
    txt += `╰➺ ✧ *Resultado:* ${trabajoSeleccionado.tipo === 'positivo' ? '+' : '-'}${cantidad} ${global.moneda}\n`
